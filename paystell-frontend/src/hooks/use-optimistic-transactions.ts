...
export function useOptimisticTransactions() {
  ...
  const createDepositTransaction = useCallback(
    (amount: string, asset: string, address: string) => {
  ...
  const createWithdrawTransaction = useCallback(
    (amount: string, asset: string, destination: string) => {
      const transaction: OptimisticTransaction = {
  ...
  const resolveSourceQueue = useCallback(
    (id: string): 'pending' | 'processing' | 'completed' | 'failed' | null => {
  ...
  const getTotalConfirmedAmount = useCallback(
    (asset?: string) => {
