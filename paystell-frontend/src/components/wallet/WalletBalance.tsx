...
export default function WalletBalance() {
  ...
  if (balanceError) {
    return <div className="p-4 border border-red-200 rounded-md bg-red-50 text-red-600">{balanceError}</div>
  }

  if (balances.length === 0) {
    return <div className="text-center py-6 text-muted-foreground">No assets found in this account.</div>
  }
  ...
  const getXlmUsdValue = (balance: string) => {
    ...
    const xlmAmount = Number.parseFloat(balance)
      ...
      {xlmBalance && (
        <div className="flex justify-between items-center p-4 border rounded-lg hover:bg-muted/50 transition-colors">
          <div className="flex items-center gap-2">
            <Image alt="xlm" src={process.env.NEXT_PUBLIC_COIN_LOGO_URL+"/512.png"} width={40} height={40} />
            <h3 className="font-medium">XLM</h3>
          </div>
          <div>
          </div>
          <div className="text-right">
            <p className="font-bold text-lg">{Number.parseFloat(xlmBalance.balance).toFixed(2)}</p>
            {!isLoadingPrice && xlmPrice && (
              <p className="text-sm text-muted-foreground">≈ ${getXlmUsdValue(xlmBalance.balance)} USD</p>
            )}
          </div>
        </div>
      )}
