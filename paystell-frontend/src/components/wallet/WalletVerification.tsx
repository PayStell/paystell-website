...
import { CheckCircle2, Wallet, Key, Mail, Loader2, ChevronRight, Shield, AlertCircle } from 'lucide-react';
    ...
    return (
        <div className="flex items-start w-[max-content] gap-3">
            ...
            <div className="flex-1">
                <h3 className={`font-medium ${currentStep === numberlabel ? 'text-blue-500' :
...
const WalletOption = ({ name, icon, onClick }: { name: string, icon: ReactNode, onClick: () => void }) => (
    <button
        onClick={onClick}
...
const WalletVerification = () => {
    ...
    const steps = [
        { numberlabel: 1, title: 'Connect Wallet', description: 'Connect your Stellar wallet to begin the verification process.', status: stage !== STAGES.CONNECT ? 'completed' : 'pending' },
        { numberlabel: 2, title: 'Sign Message', description: 'Sign a message to verify wallet ownership', status: stage === STAGES.EMAIL || stage === STAGES.CONFIRMED ? 'completed' : 'pending' },
        { numberlabel: 3, title: 'Email Verification', description: 'Verify your wallet through your registered email.', status: stage === STAGES.CONFIRMED ? 'completed' : 'pending' }
    ];
    ...
    const renderActionButton = () => {
        ...
        switch (stage) {
                            ...
                            <DialogContent className="sm:max-w-md">
                                ...
                                <div className="my-6 space-y-4">
                                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                                        <h4 className="text-sm font-medium text-gray-700 mb-2">Message to sign:</h4>
                                        <p className="text-sm text-gray-600 font-mono bg-white p-3 rounded border border-gray-200">
                                            Verify wallet ownership for account registration at {new Date().toISOString().split('T')[0]}
                                        </p>
                                    </div>
                                ...
                                <div className="flex flex-col sm:flex-row gap-3">
                                    ...
                                    <Button
                                        className="sm:w-full"
                                        onClick={() => { setIsSignDialogOpen(false); mockProcess(STAGES.EMAIL); }}
                                    >
                                        Sign Message
                                    </Button>
                                </div>
            ...
            case STAGES.EMAIL:
                return <Button onClick={() => mockProcess(STAGES.CONFIRMED)} className="w-full"><Mail className="w-4 h-4 mr-2" /> Check Email</Button>;
            default:
                return null;
        }
    };
    ...
    return (
        <Card className="w-full max-w-lg px-5 mx-auto">
            <CardHeader className="text-center">
                ...
                <p className="text-gray-500 text-sm mt-2">Complete these steps to verify your wallet</p>
            </CardHeader>
            <CardContent className="space-y-6">
                {steps.map((step) => (
                    <VerificationStep key={step.numberlabel} currentStep={steps.findIndex(s => s.status !== 'completed') + 1} {...step} />
                ))}
                {renderActionButton()}
            </CardContent>
        </Card>
