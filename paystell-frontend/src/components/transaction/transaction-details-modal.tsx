...
import { ExternalLink, Loader2, Copy, Check, ArrowUpRight, ArrowDownLeft, Clock, AlertCircle } from "lucide-react"
...
export function TransactionDetailsModal({ transactionId, onClose, network }: TransactionDetailsModalProps) {
            ...
            <div className="p-6 pt-2">
              <Tabs defaultValue="overview" className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                  ...
                  <TabsTrigger value="operations">Operations ({details.operations.length})</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="overview" className="space-y-6 pt-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm">{(Number(details.transaction.fee_charged) / 10000000).toFixed(7)} XLM</p>
                    </div>

                    <div className="space-y-1">
                      <h4 className="text-sm font-medium text-muted-foreground">Source Account</h4>
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-mono break-all">{details.transaction.source_account}</p>
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                            ...
                            <TooltipContent>
                              <p>{copiedText === details.transaction.source_account ? "Copied!" : "Copy Address"}</p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </div>
                    ...
                    <div className="space-y-1">
                      {details.transaction.signatures.map((signature, index) => (
                        <div key={index} className="flex items-center gap-2">
                          <p className="text-xs font-mono break-all bg-muted p-1.5 rounded flex-1">{signature}</p>
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                  ...
                  <div className="pt-2">
                    <Button variant="outline" size="sm" className="text-xs" asChild>
                      <a href={getExplorerUrl(details.transaction.id)} target="_blank" rel="noopener noreferrer">
                      ...
                      {details.operations.map((op, index) => (
                        <div key={op.id} className="border rounded-lg p-4 bg-card">
                          <div className="flex justify-between items-start mb-3">
                            <div className="flex items-center gap-2">
                              <div className="bg-primary/10 p-1.5 rounded-full">{getOperationIcon(op.type)}</div>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <TooltipProvider>
                                  <Tooltip>
                                    ...
                                    <TooltipContent>
                                      <p>{copiedText === op.source_account ? "Copied!" : "Copy Address"}</p>
                                    </TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
                              </div>
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <TooltipProvider>
                                      <Tooltip>
                                        ...
                                        <TooltipContent>
                                          <p>{copiedText === op.account ? "Copied!" : "Copy Address"}</p>
                                        </TooltipContent>
                                      </Tooltip>
                                    </TooltipProvider>
                                  </div>
